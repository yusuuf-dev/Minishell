...
/*check for errno in case the wait did fail for another reason other
...
static void	wait_children(t_pipes *pipe_data, t_data *data)
{
	...
	waitpid(pipe_data->cpid, &child_info, 0);
...
static void	redirecting_child_std(t_data *data, t_pipes *pipe_data)
{
	...
	if (pipe_data->i == 0)
	{
		if (dup2(pipe_data->pipefd[1], STDOUT_FILENO) == -1)
			print_free_exit(DUP_FAILED, errno);
		close_save_old_pipe(pipe_data, 2);
	}
...
int	ft_pipes(t_data *data)
{
	...
	while (pipe_data.piped_cmds[pipe_data.i])
	{
		...
		else
		{
			...
			close_save_old_pipe(&pipe_data, 1);
		}
		pipe_data.i += 1;
	}
