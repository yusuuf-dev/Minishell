...
int ft_pipes(char **piped_cmds, char **p, unsigned char *status, int *is_a_pipe)
{
    ...
    *is_a_pipe = 1;
        ...
        *p = piped_cmds[i];
        ...
        { // child   // maybe split every single pos possibelity into functs ??
            ...
            {// I probably need to redirect the std in,out before I fork so 
                if (close(pipefd_0[0]) == -1 || dup2(pipefd_0[1], STDOUT_FILENO) == -1 || close(pipefd_1[0]) == -1 ) //closing the reading end of the pipe
                    return (perror(""), errno); //redirecting std out to the writing end of the pipe
            }
            else if (piped_cmds[i + 1])
            {
                if ((close(pipefd_0[0]) == -1 || dup2(pipefd_1[0], STDIN_FILENO) == -1 || dup2(pipefd_0[1], STDOUT_FILENO) == -1))
                    return (perror(""), errno);
            }
            else
            {
                if (close(pipefd_0[0]) == -1 || close(pipefd_0[1]) == -1 || (dup2(pipefd_1[0], STDOUT_FILENO) == -1))
                    return (perror(""), errno);
            }
            return (0);
        }
        else // parent
        {
            if (!i && close(pipefd_1[1]) == -1)
                return (perror(""), errno);
            if (piped_cmds[i + 1] == NULL && (close(pipefd_0[0]) == -1))
                return (perror(""), errno);
            ...
            if (piped_cmds[i + 1]) // there's another part of the pipe
            {
                ...
                *p = piped_cmds[i + 1];
                ...
                if (cpid_2 == 0)
                {
                    ...
                    {
                        if ((dup2(pipefd_0[0], STDIN_FILENO) == -1 || close(pipefd_1[0]) == -1 || dup2(pipefd_1[1], STDOUT_FILENO) == -1))
                            return (perror(""), errno);
                    }
                    else // this is the last command in list of piped commands, no need to redirect stdout
                    {
                        if ((close(pipefd_1[0]) == -1 || close(pipefd_1[1]) == -1 || dup2(pipefd_0[0], STDIN_FILENO) == -1)) // close(pipefd_0[1]) == -1 ||
                            return (perror(""), errno);
                    }
                    return (0);
                }
                ...
                {
                    if ((close(pipefd_0[0]) == -1) || (close(pipefd_1[1]) == -1) || (pipe(pipefd_0) == -1))
                        return (perror(""), errno);
    ...
    if (pos)
    {
        if ((close(pipefd_0[0]) == -1) || (close(pipefd_0[1]) == -1) || (close(pipefd_1[0]) == -1))
            return (perror(""), errno);
    }
    else
    {
        if ((close(pipefd_1[0]) == -1) || (close(pipefd_1[1]) == -1))// || (close(pipefd_0[0]) == -1))
            return (perror(""), errno);
    }
    // close all pipes;
    free_all(piped_cmds);
